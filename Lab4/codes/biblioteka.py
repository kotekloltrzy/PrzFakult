class biblioteka():
    ...
    def Dodaj_ksiazke(self, tytul, autor, rok_wydania):
        self.ID += 1
        nowa_ksiazka = {'id': self.ID, 'tytul': tytul, 'autor': autor, 'rok_wydania': rok_wydania, 'wypozyczona': False,
                        'czytelnik': None}
    ...
    def znajdz_ksiazke(self, fraza):
        wyniki = []
        for ksiazka in self.ksiazki:
            if fraza.lower() in ksiazka['tytul'].lower() or fraza.lower() in ksiazka['autor'].lower():
